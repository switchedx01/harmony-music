...
APP_NAME = "HarmonyPlayer"
APP_AUTHOR = "HarmonyPlayer"
CACHE_BASE_DIR = Path(appdirs.user_cache_dir(APP_NAME, APP_AUTHOR))
ALBUM_ART_CACHE_DIR = CACHE_BASE_DIR / "album_art"

try:
    os.makedirs(ALBUM_ART_CACHE_DIR, exist_ok=True)
    log.info(f"Image cache directory is set to: {ALBUM_ART_CACHE_DIR}")
except OSError as e:
    log.critical(f"Failed to create cache directory at {ALBUM_ART_CACHE_DIR}: {e}")
    import tempfile
    ALBUM_ART_CACHE_DIR = Path(tempfile.gettempdir()) / "harmony_player_cache"
    os.makedirs(ALBUM_ART_CACHE_DIR, exist_ok=True)
    log.warning(f"Using temporary fallback cache directory: {ALBUM_ART_CACHE_DIR}")
    ...
    if force_overwrite or not normal_cache_path.exists():
        try:
            with Image.open(BytesIO(raw_data)) as img:
                ...
                log.debug(f"Successfully cached normal image to {normal_cache_path}")
        except Exception as e:
        ...
        if force_overwrite or not blurred_cache_path.exists():
            try:
                with Image.open(BytesIO(raw_data)) as img:
                    ...
                    log.debug(f"Successfully cached blurred image to {blurred_cache_path}")
            except Exception as e:
